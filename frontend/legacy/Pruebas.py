...
def main(page: ft.Page):
    ...
    def calcular_cambio_real(e=None):
        try:
            ...
            recibido_val = input_recibido.value.strip()
            recibido = float(recibido_val) if recibido_val else 0
            cambio = recibido - total
            
            # Lógica de bloqueo y color
            ...
            if total == 0:
                txt_cambio_display.value = "Esperando productos..."
                txt_cambio_display.color = ft.Colors.BLUE_GREY_400
            elif cambio >= 0:
    ...
    def finalizar_venta_event(e):
        ...
        # Reset visual del botón (Aquí es donde se ponía verde antes)
        btn_finalizar.disabled = True
        btn_finalizar.bgcolor = ft.Colors.GREY_400
    ...
    def actualizar_interfaz_ventas():
        ...
        calcular_cambio_real() # Al actualizar la tabla, recalculamos el botón
    ...
    txt_cambio_display = ft.Text("Esperando productos...", size=18, weight="bold", color=ft.Colors.BLUE_GREY_400)
    txt_total_v = ft.Text("Total: $0.00", size=28, weight="bold", color="blue900")
    
    # EL BOTÓN INICIA GRIS
    ...
    view_stock = ft.Container(content=ft.Column([ft.Text("📦 Inventario", size=30, weight="bold"), ft.ListView(expand=True)]), expand=True, visible=False)
    view_clientes = ft.Container(content=ft.Column([ft.Text("👥 Registro de Clientes", size=30, weight="bold"), ft.TextField(label="Nombre", icon=ft.Icons.PERSON), ft.FilledButton("Guardar", icon=ft.Icons.SAVE)]), expand=True, visible=False)

    txt_c_efectivo = ft.Text("$0.00", size=45, weight="bold", color="green")
    txt_c_tarjeta = ft.Text("$0.00", size=45, weight="bold", color="blue")
    view_corte = ft.Container(content=ft.Column([ft.Text("📊 Resumen del Día", size=35, weight="bold"), ft.Row([ft.Card(content=ft.Container(padding=40, width=400, content=ft.Column([ft.Icon(ft.Icons.ATTACH_MONEY, size=50, color="green"), ft.Text("EFECTIVO"), txt_c_efectivo], horizontal_alignment="center"))), ft.Card(content=ft.Container(padding=40, width=400, content=ft.Column([ft.Icon(ft.Icons.CREDIT_CARD, size=50, color="blue"), ft.Text("TARJETA"), txt_c_tarjeta], horizontal_alignment="center")))], alignment="center")], horizontal_alignment="center"), expand=True, visible=False)
    ...
    rail = ft.NavigationRail(selected_index=0, label_type="all", destinations=[
    ...
    login_card = ft.Card(content=ft.Container(padding=40, width=400, content=ft.Column([ft.Icon(ft.Icons.LOCK_PERSON_ROUNDED, size=80, color="blue"), ft.Text("FESConnect POS", size=28, weight="bold"), txt_user, txt_pass, ft.FilledButton("Entrar", on_click=login, width=250)], horizontal_alignment="center", spacing=15)))
