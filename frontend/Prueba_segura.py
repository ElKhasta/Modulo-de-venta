...
def main(page: ft.Page):
    ...
    def calcular_cambio_real(e=None):
        try:
            ...
            btn_finalizar.bgcolor = ft.Colors.GREY_400
        page.update()
    ...
    def finalizar_venta_event(e):
        ...
        # Limpieza y reset visual del botón
        carrito.clear()
    ...
    view_stock = ft.Container(content=ft.Column([ft.Text("📦 Inventario", size=30, weight="bold"), ft.Row([ft.TextField(label="Filtrar...", expand=True), ft.IconButton(ft.Icons.SEARCH)]), ft.ListView(expand=True)]), expand=True, visible=False)
    view_clientes = ft.Container(content=ft.Column([ft.Text("👥 Registro de Clientes", size=30, weight="bold"), ft.TextField(label="Nombre / Razón Social", icon=ft.Icons.PERSON), ft.TextField(label="RFC", icon=ft.Icons.BADGE), ft.TextField(label="Email", icon=ft.Icons.EMAIL), ft.FilledButton("Guardar", icon=ft.Icons.SAVE, color="white", bgcolor="blue")]), expand=True, visible=False)

    txt_c_efectivo = ft.Text("$0.00", size=45, weight="bold", color="green")
    txt_c_tarjeta = ft.Text("$0.00", size=45, weight="bold", color="blue")
    view_corte = ft.Container(content=ft.Column([ft.Text("📊 Resumen del Día", size=35, weight="bold"), ft.Row([ft.Card(content=ft.Container(padding=40, width=400, content=ft.Column([ft.Icon(ft.Icons.ATTACH_MONEY, size=50, color="green"), ft.Text("EFECTIVO", size=20), txt_c_efectivo], horizontal_alignment="center"))), ft.Card(content=ft.Container(padding=40, width=400, content=ft.Column([ft.Icon(ft.Icons.CREDIT_CARD, size=50, color="blue"), ft.Text("TAR_TRANS", size=20), txt_c_tarjeta], horizontal_alignment="center")))], alignment=ft.MainAxisAlignment.CENTER, spacing=30), ft.FilledButton("Cerrar Caja", icon=ft.Icons.LOCK_OUTLINE, scale=1.2)], horizontal_alignment=ft.CrossAxisAlignment.CENTER, alignment=ft.MainAxisAlignment.CENTER, spacing=40), expand=True, visible=False)
    ...
    def cambiar_tab(e):
        ...
        view_ventas.visible = (idx == 0); view_stock.visible = (idx == 1); view_clientes.visible = (idx == 2); view_corte.visible = (idx == 3)
        if idx == 3:
    ...
    rail = ft.NavigationRail(selected_index=0, label_type="all", min_width=100, destinations=[
    ...
    def login(e):
        ...
        try:
            #Usamos POST y enviamos el JSON con usuario y contraseña
            res = requests.post(
                f"{BASE_URL}/login/", 
            ...
            # Si el servidor responde 200 o 201, las credenciales son válidas
            if res.status_code in [200, 201]:
                cargar_principal()
            elif res.status_code == 400 or res.status_code == 401:
                mostrar_snack("❌ Usuario o contraseña incorrectos")
            else:
                mostrar_snack(f"Error en servidor: {res.status_code}")
                
        except Exception as ex:
            print(f"Error de red: {ex}")
            mostrar_snack("🌐 Error de red: El servidor no responde")

    login_card = ft.Card(content=ft.Container(padding=40, width=400, content=ft.Column([ft.Icon(ft.Icons.LOCK_PERSON_ROUNDED, size=80, color="blue"), ft.Text("FESConnect POS", size=28, weight="bold"), ft.Divider(height=20, color="transparent"), txt_user, txt_pass, ft.Divider(height=10, color="transparent"), ft.FilledButton("Entrar", on_click=login, width=250)], horizontal_alignment="center", spacing=15)), elevation=10)
